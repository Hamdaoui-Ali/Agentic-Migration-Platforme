...
import type { JavaRouteDisposition, JavaRouteStage } from "../domain/types";

type JavaStageDisplayStatus = JavaRouteDisposition | "DONE" | "RUNNING" | "REPAIR";
...
export function JavaRouteBoard({
  ...
  return (
    <div className="space-y-2">
      {route.map((stage) => {
        const status: JavaStageDisplayStatus = completedStages.includes(stage.stage)
          ? "DONE"
          : activeStage === stage.stage && activeStatus
            ? activeStatus
            : stage.disposition;
            ...
            <StatusBadge
              ...
              tone={
                status === "DONE"
                  ...
                  : status === "REPAIR"
                    ? "danger"
                    : status === "RUNNING" || status === "INCLUDED"
                      ? "info"
                      : status === "SKIPPED"
                        ? "neutral"
                        : "warning"
