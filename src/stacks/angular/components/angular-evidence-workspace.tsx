...
export function AngularEvidenceWorkspace({ run }: { run: AngularRunModel }) {
  ...
  const visible = filter === "ALL" ? run.evidence : run.evidence.filter((item) => item.category === filter);
  ...
  return (
    <Panel>
      <PanelHeader
        ...
        description="Decision and execution evidence remains append-only as the workflow advances."
      />
      ...
      <div className="mt-5 overflow-hidden rounded-lg border border-[var(--mf-border)]">
        ...
        {visible.length === 0 ? (
          <div className="p-8 text-center text-sm text-[var(--mf-text-muted)]">No evidence in this category yet.</div>
        ) : null}
      </div>
    </Panel>
  );
...
function EvidenceRow({ item }: { item: AngularRunEvidence }) {
  return (
    <article className="border-b border-[var(--mf-border)] p-4 last:border-b-0">
      <div className="flex flex-col justify-between gap-3 md:flex-row md:items-start">
        <div>
          <p className="text-[10px] font-bold uppercase tracking-[0.08em] text-[var(--mf-text-soft)]">{item.category}</p>
